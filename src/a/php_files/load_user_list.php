...
if(!isset($_POST['mode'])||!isset($_POST['se'])||!isset($_POST['str'])||!isset($_POST['stp'])){
    ...
    $m='';
    if($md=='a'){
        $m='`Locked`=false and `Deleted`=false and';
    }elseif($md=='l'){
        $m='`Locked`=true and';
    }elseif($md=='d'){
        $m='`Deleted`=true and';
    ...
    $tb;
    $lm;
    $sql="select * from `user` where $m (`ID` like '%$se%' or `FirstName` like '%$se%' or LastName like '%$se%' or `Email` like '%$se%') $lm;";
    ...
    if($rc>0){
        while($row=mysqli_fetch_assoc($result)){
            ...
            $tb.='<td><button id=\'btview'.$row['ID'].'\' onclick="viewUser('.$row['ID'].')" type="button" class="btn btn-primary btn-sm">View</button>
